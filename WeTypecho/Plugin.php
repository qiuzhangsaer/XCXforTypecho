...
class WeTypecho_Plugin implements Typecho_Plugin_Interface
{
    ...
    {        
        $swipePosts = new Typecho_Widget_Helper_Form_Element_Text('swipePosts', NULL, '1,2', _t('banner显示文章列表'),  _t('要在banner列表里面显示的文章的cid值，用英文逗号隔开。'));
        $form->addInput($swipePosts);
        $apiSecret = new Typecho_Widget_Helper_Form_Element_Text('apiSecret', NULL, 'xxx', _t('API密钥'),  _t('要与小程序端app.js中的config里的apisec字段保持一致，否则无法从服务器读取数据'));
        ...
        $aboutCid = new Typecho_Widget_Helper_Form_Element_Text('aboutCid', NULL, '1', _t('关于页面CID'),  _t('小程序关于页面显示内容的文章id'));
